...
CREATE TABLE public.aggregate_events_01_image (
                                                     id integer DEFAULT nextval('public.aggregate_events_01_image_id_seq') NOT NULL,
                                                     aggregate_id uuid NOT NULL,
                                                     event_id integer
);
...
create index ix_01_events_image_id on public.events_01_image(aggregate_id);
create index ix_01_aggregate_events_image_id on public.aggregate_events_01_image(aggregate_id);
create index ix_01_snapshot_image_id on public.snapshots_01_image(aggregate_id);
create index ix_01_events_image_timestamp on public.events_01_image("timestamp");
create index ix_01_snapshots_image_timestamp on public.snapshots_01_image("timestamp");

CREATE OR REPLACE FUNCTION insert_01_image_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_image_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
CREATE OR REPLACE FUNCTION insert_01_image_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_image_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text   
)
