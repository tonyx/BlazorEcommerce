...
CREATE TABLE public.aggregate_events_01_address (
                                                    id integer DEFAULT nextval('public.aggregate_events_01_address_id_seq') NOT NULL,
                                                    aggregate_id uuid NOT NULL,
                                                    event_id integer
);
...
create index ix_01_events_address_id on public.events_01_address(aggregate_id);
create index ix_01_aggregate_events_address_id on public.aggregate_events_01_address(aggregate_id);
create index ix_01_snapshot_address_id on public.snapshots_01_address(aggregate_id);

CREATE OR REPLACE FUNCTION insert_01_address_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_address_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
CREATE OR REPLACE FUNCTION insert_01_address_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_address_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text   
)
