...
CREATE TABLE public.aggregate_events_01_cartItem (
                                                     id integer DEFAULT nextval('public.aggregate_events_01_cartItem_id_seq') NOT NULL,
                                                     aggregate_id uuid NOT NULL,
                                                     event_id integer
);
...
create index ix_01_events_cartItem_id on public.events_01_cartItem(aggregate_id);
create index ix_01_aggregate_events_cartItem_id on public.aggregate_events_01_cartItem(aggregate_id);
create index ix_01_snapshot_cartItem_id on public.snapshots_01_cartItem(aggregate_id);

CREATE OR REPLACE FUNCTION insert_01_cartItem_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_cartItem_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
CREATE OR REPLACE FUNCTION insert_01_cartItem_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_cartItem_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text   
)
