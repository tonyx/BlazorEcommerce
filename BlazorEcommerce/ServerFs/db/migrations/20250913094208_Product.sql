...
CREATE TABLE public.aggregate_events_01_productType (
                                                  id integer DEFAULT nextval('public.aggregate_events_01_productType_id_seq') NOT NULL,
                                                  aggregate_id uuid NOT NULL,
                                                  event_id integer
);
...
create index ix_01_events_productType_id on public.events_01_productType(aggregate_id);
create index ix_01_aggregate_events_productType_id on public.aggregate_events_01_productType(aggregate_id);
create index ix_01_snapshot_productType_id on public.snapshots_01_productType(aggregate_id);
create index ix_01_events_product_timestamp on public.events_01_productType("timestamp");
create index ix_01_snapshots_product_timestamp on public.snapshots_01_productType("timestamp");

CREATE OR REPLACE FUNCTION insert_01_productType_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_productType_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
CREATE OR REPLACE FUNCTION insert_01_productType_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_productType_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text   
)
