...
CREATE TABLE public.aggregate_events_01_orderItem (
                                                  id integer DEFAULT nextval('public.aggregate_events_01_orderItem_id_seq') NOT NULL,
                                                  aggregate_id uuid NOT NULL,
                                                  event_id integer
);
...
create index ix_01_events_orderItem_id on public.events_01_orderItem(aggregate_id);
create index ix_01_aggregate_events_orderItem_id on public.aggregate_events_01_orderItem(aggregate_id);
create index ix_01_snapshot_orderItem_id on public.snapshots_01_orderItem(aggregate_id);
create index ix_01_events_orderItem_timestamp on public.events_01_orderItem("timestamp");
create index ix_01_snapshots_orderItem_timestamp on public.snapshots_01_orderItem("timestamp");

CREATE OR REPLACE FUNCTION insert_01_orderItem_event_and_return_id(
    IN event_in text,
...
CREATE OR REPLACE FUNCTION insert_md_01_orderItem_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text
)
...
CREATE OR REPLACE FUNCTION insert_01_orderItem_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid 
)
RETURNS int
...
CREATE OR REPLACE FUNCTION insert_md_01_orderItem_aggregate_event_and_return_id(
    IN event_in text,
    IN aggregate_id uuid,
    IN md text   
)
